...
function(sanity_require_gtest version_arg)
    ...
    set(GTEST_FOUND TRUE CACHE BOOL "set by sanity" FORCE)
    set(GTEST_INCLUDE_DIRS "${sanity.target.local}/include" CACHE PATH "set by sanity" FORCE)
    set(GTEST_INCLUDE_DIR "${sanity.target.local}/include" CACHE PATH "set by sanity" FORCE)
    set(libs "${sanity.target.local}/lib/libgtest.a")
    if (CMAKE_THREAD_LIBS_INIT)
        list(libs ${CMAKE_THREAD_LIBS_INIT})
    endif ()
    set(GTEST_LIBRARIES "${libs}" CACHE PATH "set by sanity" FORCE)
    set(GTEST_MAIN_LIBRARIES "${sanity.target.local}/lib/libgtest_main.a" CACHE PATH "set by sanity" FORCE)
    set(GTEST_BOTH_LIBRARIES ${GTEST_MAIN_LIBRARIES} ${GTEST_LIBRARIES} CACHE PATH "set by sanity" FORCE)
    set(GTEST_LIBRARY_DIRS "${sanity.target.local}/lib" CACHE PATH "set by sanity" FORCE)
