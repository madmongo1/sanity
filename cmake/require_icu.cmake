...
error code : ${res}"
    ...
    set(ICU_LIBRARY_DIR "${sanity.target.local}/lib")
    set(ICU_LIBRARIES ${ICU_LIBRARY_DIR}/libicui18n.a ${ICU_LIBRARY_DIR}/libicuuc.a ${ICU_LIBRARY_DIR}/libicudata.a)
    ...
    foreach (file IN LISTS ICU_LIBRARIES)
        ...
        if (NOT TARGET "${sanity_lib}")
            ...
            target_link_libraries("${sanity_lib}" INTERFACE "${file}")
            set_property(TARGET sanity::icu
                    APPEND
                    PROPERTY INTERFACE_INCLUDE_DIRECTORIES ${ICU_INCLUDE_DIRS})
            if (NOT "${component}" STREQUAL "test")
                target_link_libraries(sanity::icu INTERFACE "${sanity_lib}")
            endif ()
        endif ()
