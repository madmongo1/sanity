...
function(sanity_require_curl given_version)
    ...
    sanity_require(LIBRARY c-ares VERSION any)
    ...
    if (${untar_flag} IS_NEWER_THAN ${configure_flag}
            OR "${CMAKE_CURRENT_LIST_FILE}" IS_NEWER_THAN "${configure_flag}"
        ...
        list(APPEND configure_command "--enable-ares=${CARES_ROOT}")
        list(APPEND configure_command "--without-zlib")
        sanity_join(command " " ${configure_command})
...
error code : ${res}"
    ...
    if (NOT TARGET curl)
        ...
        target_link_libraries(curl
                INTERFACE ${CURL_LIBRARIES} ${OPENSSL_LIBRARIES} ${CARES_LIBRARIES})
        set_property(TARGET curl
                APPEND
                PROPERTY INTERFACE_INCLUDE_DIRECTORIES ${CURL_INCLUDE_DIRS} ${OPENSSL_INCLUDE_DIR} ${CARES_INCLUDE_DIR})
    endif ()
